...
- [Яндекс](#яндекс)
  - [Очистка рекламы](#очистка-рекламы)
...
Пока работает на `translate.yandex.ru` и `translate.yandex.com`.
...
## [YouTube TXT](https://github.com/nikmedoed/myTampermonkeyScripts/raw/main/YouTube-txt.user.js)

Скрипт добавляет компактную кнопку **TXT** в верхнюю панель YouTube на страницах видео и скачивает расшифровку текущего ролика в `.txt` файл.

Сначала скрипт пытается получить текст через интерфейс расшифровки YouTube. Если такой панели нет или она не отдала текст, используется запасной способ через доступные дорожки субтитров из данных плеера. Файл сохраняется с названием видео, без таймкодов и с нормализованными пробелами.

![YouTube TXT](icons/youtube-txt.png)

### Установка<!-- omit from toc -->

1. Установите расширение [Tampermonkey®](https://www.tampermonkey.net/).
2. Перейдите по [ссылке](https://github.com/nikmedoed/myTampermonkeyScripts/raw/main/YouTube-txt.user.js) и установите скрипт.

### Ограничения<!-- omit from toc -->

Скрипт работает только на страницах вида `youtube.com/watch?v=...`. Если у видео нет доступной расшифровки или субтитров, скачать текст не получится.
